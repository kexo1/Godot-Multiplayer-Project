...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed("ui_cancel") and DisplayServer.window_is_focused():
		if !$title.visible and !$death_screen.visible:
...
func update_health_label(current_health: String) -> void:
	$ingame_ui/health.text = current_health

func _on_server_connect() -> void:
	_off()
	$ingame_ui.visible = true

func set_death_screen(dead: bool) -> void:
	_off()
	$death_screen.visible = !dead
	$ingame_ui.visible = dead

func _get_local_ip_addresses():
	for address in IP.get_local_addresses():
		if ":" not in address:
			$title/main_menu/host_screen/ip_addresses.add_item(address)

func _on_death_screen_respawn_pressed() -> void:
	GameData.player_controller_link.respawn_player()

func _on_death_screen_leave_pressed() -> void:
...
func _debug() -> void:
	for arg in OS.get_cmdline_args():
		if arg == "s":
			HighLevelNetworkHandler.start_server(6969)
		if arg == "c":
			HighLevelNetworkHandler.start_client(6969, "localhost")
